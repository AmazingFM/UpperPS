...
+ (NSArray *)objectArrayWithKeyValuesArray:(id)keyValuesArray context:(NSManagedObjectContext *)context error:(NSError *__autoreleasing *)error
{
    ...
    NSMutableDictionary *keyValuesCopy;
    // 3.遍历
    for (NSDictionary *keyValues in keyValuesArray) {
        NSArray *keyArr = keyValues.allKeys;
        NSMutableDictionary *keyValuesCopy = [NSMutableDictionary dictionaryWithDictionary:keyValues];
        if ([keyArr containsObject:@"id"]) {
            [keyValuesCopy setValue:keyValues[@"id"] forKey:@"ID"];
        }
        id model = [self objectWithKeyValues:keyValuesCopy context:context error:error];
