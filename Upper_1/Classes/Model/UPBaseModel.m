...
//  Upper
//
//  Created by freshment on 16/5/30.
...
@interface UPBaseModel ()

@end

@implementation UPBaseModel
...
+ (NSArray *)jsonArrayWithObjectArray:(NSArray *)objectArray {
    ...
    for (UPBaseModel *model in objectArray) {
        ...
        if ([self respondsToSelector:setter]) {
            ...
            if ([aDictValue isKindOfClass:[NSDictionary class]]) {
                ...
                if ([type isEqualToString:@"NSDictionary"] == NO && [type isEqualToString:@"NSMutableDictionary"] == NO) {
                    ...
                    if ([newClass isSubclassOfClass:[UPBaseModel class]]) {
                ...
                if (classString) {
                    for (id content in aDictValue) {
                        ...
                        //如果是该类的子类，说明能响应initWithDict:方法并且能建立映射字典。
                        if ([classInArray isSubclassOfClass:[UPBaseModel class]]) {
...
- (NSDictionary *)dictionaryRepresentation {
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (key && value) {
            if ([value isKindOfClass:[UPBaseModel class]]) {
